...
document.addEventListener('DOMContentLoaded', function() {
	document.querySelectorAll('.btn-load-more').forEach(function(button) {
		button.addEventListener('click', function(e) {
			...
			var section = button.closest('[data-loadmore-max-pages], .msrawards-stories-list, section');
			var cfg = section
				? {
			...
			fetch(params.ajaxurl, {
				...
				.then(function(html) {
					...
					if (hasPosts) {
						var wrapSection = button.closest('.msrawards-stories-list, section');
						...
						if (wrap) {
							...
							if (typeof window.msrawardsRevealNodes === 'function') {
								window.msrawardsRevealNodes(wrap);
							}
						}
						cfg.page = page + 1;
						if (section) {
