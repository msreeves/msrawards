...
$all_label     = isset( $args['all_label'] ) ? (string) $args['all_label'] : __( 'All', 'msrawards' );
...
$empty_message = isset( $args['empty_message'] ) ? (string) $args['empty_message'] : __( 'No items found in this category.', 'msrawards' );
$filter_label  = isset( $args['filter_label'] ) ? (string) $args['filter_label'] : __( 'Filter listings', 'msrawards' );

$filter_terms = msrawards_get_filter_terms( $taxonomy, $parent );
$tabs_id      = 'msr-filter-' . sanitize_html_class( $post_type . '-' . $taxonomy );
$all_query    = new WP_Query(
	msrawards_filter_tabs_query_args( $post_type, '', $taxonomy, $query_args )
);
?>
<div class="post-tabs msr-filter-tabs" id="<?php echo esc_attr( $tabs_id ); ?>" data-msr-filter-tabs>
	<?php
	msrawards_filter_bar_open( $filter_label, true );
	msrawards_filter_bar_tab( $all_label, $tabs_id . '-tab-all', $tabs_id . '-pane-all', true );
	foreach ( $filter_terms as $term ) {
		if ( ! ( $term instanceof WP_Term ) ) {
			continue;
		}
		msrawards_filter_bar_tab(
			$term->name,
	...
	msrawards_filter_bar_close();
	msrawards_filter_bar_status( $all_label );
	?>
	...
	<div class="tab-content">
		...
		>
			<?php if ( $all_query->have_posts() ) : ?>
				<div class="row msr-card-grid g-3 align-items-stretch">
					<?php
		...
		<?php foreach ( $filter_terms as $term ) : ?>
			...
			$term_query = new WP_Query(
				msrawards_filter_tabs_query_args( $post_type, $term->slug, $taxonomy, $query_args )
			);
			?>
			<div
				class="tab-pane fade"
