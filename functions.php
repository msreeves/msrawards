<?php
/**
 * msrawards functions and definitions
 *
 * @package msrawards
 */
 ...
 */
function msrawards_acf_image_url( $value ) {
	if ( '' === $value || null === $value ) {
		return '';
	}
	if ( is_numeric( $value ) ) {
	...
	if ( is_array( $value ) ) {
		...
		if ( ! empty( $value['ID'] ) ) {
			return msrawards_acf_image_url( $value['ID'] );
		}
		return '';
	}
	if ( is_string( $value ) ) {
		...
		if ( ctype_digit( $trim ) ) {
			return msrawards_acf_image_url( (int) $trim );
		}
		$maybe = maybe_unserialize( $trim );
		if ( $maybe !== $trim && ( is_array( $maybe ) || is_numeric( $maybe ) ) ) {
			return msrawards_acf_image_url( $maybe );
		}
		return $trim;
	}
	return '';
...
function msrawards_setup() {
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'msrawards_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'msrawards_setup' );
...
if( function_exists('acf_add_options_page') ) {
    ...
    acf_add_options_page(array(
        'page_title'    => 'Award',
        'menu_title'    => 'Award Information',
        'menu_slug'     => 'award-information',
