...
if ( '' === $title ) {
	switch ( $context ) {
		case 'search':
			$title = __( 'No results for that search', 'msrawards' );
			break;
		case 'archive':
			$title = __( 'Nothing published here yet', 'msrawards' );
			break;
		case 'gallery':
			$title = __( 'Gallery coming soon', 'msrawards' );
			break;
		default:
			$title = __( 'Nothing to show yet', 'msrawards' );
	}
}
...
if ( '' === $message ) {
	switch ( $context ) {
		case 'search':
			$message = __( 'Try different keywords or browse nominees and programme news from the awards home.', 'msrawards' );
			$search  = true;
			break;
		case 'archive':
			$message = __( 'Check back after the next nomination round, or return to the awards home.', 'msrawards' );
			break;
		case 'gallery':
			$message = __( 'Gallery images will appear here when added in the page editor.', 'msrawards' );
			break;
		default:
			$message = __( 'Content will appear here when published in the admin.', 'msrawards' );
	}
}

if ( ! $links && ! $inline ) {
	$links = msrawards_get_empty_state_default_links();
}
...
<div class="<?php echo esc_attr( $classes ); ?>" data-msr-empty-state="<?php echo esc_attr( $context ); ?>" role="status">
	...
	<?php endif; ?>
		...
		<?php endif; ?>
		<?php if ( $links && ! $inline ) : ?>
		<nav class="msr-empty-state__links d-flex flex-wrap gap-2 justify-content-center mt-3" aria-label="<?php esc_attr_e( 'Helpful links', 'msrawards' ); ?>">
