...
$category_links = msrawards_get_post_category_links_html( get_the_ID(), $category_depth );
?>
<div class="col-xl-4 col-lg-4">
	<article <?php post_class( 'post-card post panel msr-reveal msr-reveal--up' ); ?>>
		<?php msrawards_render_card_media(); ?>
		<div class="post-card__body listing-text">
			...
			<?php endif; ?>
			<?php if ( msrawards_is_sponsored_post() ) : ?>
				<span class="sponsored"><?php esc_html_e( 'This is Sponsored content', 'msrawards' ); ?></span>
			<?php endif; ?>
			<?php if ( 'search' === $variant ) : ?>
				<h2 class="h4 post-card__title"><?php echo msrawards_search_title_highlight(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h2>
				<div class="post-card__excerpt"><?php echo msrawards_search_excerpt_highlight(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
			<?php else : ?>
			...
			<a class="btn btn-primary btn-sm" href="<?php echo esc_url( get_permalink() ); ?>">
				<?php esc_html_e( 'Read more', 'msrawards' ); ?>
			</a>
		</div>
	</article>
</div>
